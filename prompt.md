...
我現在想要擴充我的地圖樣式，你可以推薦給我一點素材網站嗎，因為我是使用tilemap去做的請你提供給我一些相關素材並且我是要寫實的那種

要是免費，幫我找森林的素材

可以不是遊戲素材網的，也可以是普通的素材網站。

我還想要多一點的地圖物件，像是樹之類的物件

我現在要進入測試的場景但他會一直把我傳送到死亡場景，是因為我的測試場景沒有綁任何的scenesManager嗎，請你幫我看一下

可以請你提供7個恐怖遊戲會出現的場景嗎

我需要找到各種的場景素材，像是樹、燈、病床......等等的東西

我希望可以找不是像素的素材

我要如何使用assetstore.unity，我找到我要的素材了，但我點選Add to My Assets時卻無法加入它會跳出這個畫面，我點了兩個都沒有成功加入![alt text](image.png)

沒有我要的素材![alt text](image-1.png)

為何放進去會是紫色的![alt text](image-2.png)

用方法二變成全白的了

沒有任何地方可以放

我要如何調整這種圖片的亮度![alt text](image-3.png)

你幫我統整出這款遊戲的遊玩方式
 
請你用普通的說明就好，不用像這樣條列出來

![alt text](image-4.png)大概這個格子的大小的說明
...
使用 WASD 移動角色，滑鼠控制朝向。按下左鍵可以攻擊敵人或與物品互動。數字鍵 1-5 或滑鼠滾輪可切換快捷欄裝備。這個部分上面給米的圖片本來就有，所以不用，改其他的像是要躲避殭屍的攻擊

我現在新增一個物件，他會自帶光源，我要如何設置
 
我想加入閃爍的光源，請你幫我看一下

我現在想要新增玩家的手電筒的照射扇形範圍會依據zombie的靠近去做縮小，在某段距離時手電筒光的顏色會變成紅色的

我要再做一個有光源的物件但它是長條型的 

你能幫我找到關於門的程式碼嗎，因為遇到了一點bug，就是我改變了門的圖片後打開門時圖片會變形，請你幫我看一下

關於shadow casters 2D的設定，我想要他有陰影又想要它可以照亮那一個物件，要怎麼設定

我調整成這樣後他只有邊緣有亮而已  

我現在要新增一個東西，就是在武器開火時我要在槍的前面加入火光，我的火光已經是一個圖片，但我還要加上一點亮光，請你幫我看一下


Assets\Scripts\Weapons\Pistol.cs(196,80): error CS0104: 'Random' is an ambiguous reference between 'UnityEngine.Random' and 'System.Random'


我想要他在地板上時，不要顯示火光

不是的，因為在在 MuzzleFlash物件上加入 Sprite Renderer將Fire.png拖入 Sprite 欄位的時候，在地板上後就會顯示火光，我想要的是射擊時才顯示

我想要設定火光的位置，就是在Inspector中設定一個位置
